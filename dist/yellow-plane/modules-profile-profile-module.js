...
var FriendsComponent = /** @class */ (function () {
    function FriendsComponent(userService, route) {
        this.userService = userService;
        this.route = route;
        this.friends = [];
    }
    FriendsComponent.prototype.ngOnInit = function () {
        ...
        this.userService.getFriendRelationsByUsername(this.username).subscribe(function (relations) {
            relations.forEach(function (relation) {
                if (relation.sender === _this.username) {
                    _this.friends.push({ name: relation.reciever, location: 'Lahore', counts: { friends: 52, photos: 240, videos: 16 }, bio: 'Hi!, I’m Hassan and I’m a Community Manager for “abc”. Gamer and full time designer.', friendsSince: 'December 2014' });
                }
                if (relation.reciever === _this.username) {
                    _this.friends.push({ name: relation.sender, location: 'Lahore', counts: { friends: 52, photos: 240, videos: 16 }, bio: 'Hi!, I’m Hassan and I’m a Community Manager for “abc”. Gamer and full time designer.', friendsSince: 'December 2014' });
                }
            });
...
var TimelineComponent = /** @class */ (function () {
    function TimelineComponent(hobbiesAndInterestsService, personalInfoService, route, postService, photoService, router) {
        var _this = this;
        this.hobbiesAndInterestsService = hobbiesAndInterestsService;
        this.personalInfoService = personalInfoService;
        this.route = route;
        ...
        router.events.subscribe(function (val) {
            if (_this.username !== _this.route.snapshot.parent.params['username']) {
                _this.ngOnInit();
            }
        });
    }
    TimelineComponent.prototype.ngOnInit = function () {
        ...
        this.personalInfoService.getPersonalInfoByUserName(this.username)
            .then(function (data) { return _this.personalInfo = data; })
            .catch(function (err) { return console.log(err); });
        this.postService.getUserPostsByUsername(this.username).subscribe(function (posts) {
            _this.posts = posts;
            _this.posts.forEach(function (post, index, postsArray) {
                ...
                _this.postService.getCommentsByPostId(post.id).subscribe(function (comments) {
                    var _comments = [];
                    _comments = comments;
                    _comments.forEach(function (comment, index, commentsArray) {
                        _this.postService.getRepliesByCommentId(comment.id).subscribe(function (replies) {
    ...
    TimelineComponent.prototype.likePost = function (post) {
    ...
    TimelineComponent.prototype.postComment = function (post, comment) {
        this.postService.commentPost(post.id, comment).then(function (res) {
            console.log(res);
        }).catch(function (err) { return console.log(err); });
    };
    TimelineComponent.prototype.commentReply = function (comment, reply) {
        this.postService.replyComment(comment.id, reply).then(function (res) {
            console.log(res);
        }).catch(function (err) { return console.log(err); });
    };
    TimelineComponent.prototype.likeComment = function (comment) {
    ...
    TimelineComponent.prototype.likeReply = function (reply) {
