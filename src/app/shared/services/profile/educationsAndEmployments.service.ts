...
export class EducationsAndEmploymentsService {
    ...
    getEducationsAndEmployments() {
        return new Promise((resolve, reject) => {
            this.userService.getCurrentUser().then(user => {
                this.db.doc<EducationsAndEmployments>(`educations_and_employments/${user.uid}`).ref.get()
                .then(response => {
    ...
    public updateEducationsAndEmployments(data: EducationsAndEmployments) {
        return new Promise((resolve, reject) => {
            this.userService.getCurrentUser().then(user => {
                const educationsAndEmploymentsRef: AngularFirestoreDocument<any> = this.db.doc(`educations_and_employments/${user.uid}`);
